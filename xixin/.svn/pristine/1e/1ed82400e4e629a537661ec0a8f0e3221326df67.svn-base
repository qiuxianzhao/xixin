...
$('.base_right a').click(function(){
  ...
  /*选项卡*/
    ...
    /*页面滚动*/
    var goTop = document.getElementsByClassName('kuohao')[0];
    window.onscroll = function(){  //页面滚动事件
      var top = document.body.scrollTop; //页面滚动的值，body向上滚动的距离
      console.log( top,window.innerHeight  ); //内容显示的高度
    }
        var t1;
    goTop.onclick = function(){
      ...
      t1 = setInterval(function(){
        ...
        var v = (target-current)/6;
        v = v>0?Math.ceil(v):Math.floor(v);
        console.log(v);
        if(v!=0){
          document.body.scrollTop = current + v;
        }else{
          clearInterval(t1);
        }
      },15);    
